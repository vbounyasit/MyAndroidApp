...
interface ChatApiService : ApiService {
    ...
    @POST("/chats/logs")
    suspend fun createChatLogs(
        @Header("Authorization") authorization: String,
        @Body createChatLogsRequest: List<CreateChatLogRequest>
    ): List<CreateOperationResponse>

    @PATCH("/chats/{remoteId}/read")
    suspend fun readChat(
        @Header("Authorization") authorization: String,
        @Path("remoteId") chatRemoteId: String
    ): ReadChatResponse
    ...
    @POST("/chats/get")
    suspend fun getChatItems(
        @Header("Authorization") authorization: String,
        @Body remoteIds: List<String>
    ...
    suspend fun getChatBubbles(
        @Header("Authorization") authorization: String,
        @Path("remoteId") chatRemoteId: String
    ): ChatBubblesResponse

    @POST("/chats/logs/get")
    suspend fun getNewChatLogs(
        @Header("Authorization") authorization: String,
        @Body remoteIds: List<String>
    ): List<ChatLogResponse>

    @POST("/chats/notifications/get")
    suspend fun getNewChatNotifications(
        @Header("Authorization") authorization: String,
        @Body chatNotificationRemoteIds: List<String>
    ): List<ChatNotificationResponse>
