...
object ChatItemMapper : DTOContextMapper<ChatItem, ChatItemDTO>, NetworkResponseMapper<ChatItemResponse, ChatItem> {
    ...
    override fun toDTO(context: Context): (entity: ChatItem) -> ChatItemDTO =
        { entity ->
            val pictures = entity.profilePicture.split(context.getString(R.string.profile_pictures_delimiter))
            val lastActive = Utils.toActivityStatus(context, System.currentTimeMillis(), entity.lastActive)
            ChatItemDTO(
                entity.remoteId,
                entity.name,
                pictures,
