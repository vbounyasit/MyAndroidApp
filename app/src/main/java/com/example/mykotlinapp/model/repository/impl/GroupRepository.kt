...
class GroupRepository @Inject constructor(
    ...
    fun getGroup(groupRemoteId: String): Flow<GroupDTO?> =
        groupDao.getGroupFlow(groupRemoteId).distinctUntilChanged().map { it?.let((GroupMapper::toDTO)(context)) }
    ...
    fun getUserPostAppBar(groupRemoteId: String): Flow<PostGroupData?> =
        groupDao.getGroupFlow(groupRemoteId).distinctUntilChanged().map { it?.let((GroupMapper::toPostGroupData)(context)) }
        ...
        withContext(dispatcher) {
            ...
            group?.let { foundGroup ->
                updateGroupInput.name?.let { inputName ->
                    ...
                    chat?.let { foundChat ->
                        chatDao.update(UpdateChatMapper.toLocalUpdateWithInput(UpdateChatInput(inputName))(foundChat))
                    }
                }
                groupDao.update(UpdateGroupMapper.toLocalUpdateWithInput(updateGroupInput)(foundGroup))
            }
        }
    }
    ...
    suspend fun sendReadGroup(groupRemoteId: String): Result<Unit> {
        return withContext(dispatcher) {
            sharedPreferenceDao.performAPIAuthenticatedAction { authHeader ->
                val readGroupResponse: ReadGroupResponse = apiService.readGroup(authHeader, groupRemoteId)
                groupDao.readGroup(readGroupResponse.groupRemoteId, readGroupResponse.lastGroupReadTime)
            }
        }
    }
        ...
        return withContext(dispatcher) {
            sharedPreferenceDao.performAPIAuthenticatedAction { authHeader ->
                ...
                if (groupsToUpdate.isNotEmpty()) {
                    val request: List<UpdateGroupRequest> = groupsToUpdate.map { UpdateGroupMapper.toNetworkRequest(it) }
                    val response: UpdateOperationResponse = apiService.updateGroups(authHeader, request)
