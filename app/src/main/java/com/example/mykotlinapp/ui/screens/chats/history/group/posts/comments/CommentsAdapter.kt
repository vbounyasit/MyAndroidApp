...
class CommentsAdapter(
    ...
    bindData = { property, binding ->
        ...
        val depthFlags = property.depthFlags.split(context.resources.getString(R.string.profile_pictures_delimiter)).map { it.toBoolean() }
        ...
        binding.userCommentCard.post {
            if (depth < 0) {
                ...
                val layoutParams = RelativeLayout.LayoutParams(binding.verticalReplyLine.width, binding.verticalReplyLine.height)
                ...
                val marginFactor = context.resources.getDimensionPixelSize(R.dimen.intermediate_spacing)
                var finalMargin = startMargin
                if (depth > 0)
                    for (i in 0 until depth) {
                        if (depthFlags[i]) {
                            //creating reply vertical line
                            val newParams = RelativeLayout.LayoutParams(context.resources.getDimensionPixelSize(R.dimen.reply_line_width), RelativeLayout.LayoutParams.WRAP_CONTENT)
                            ...
                            newLine.background = context.resources.getDrawable(R.color.reply_line, null)
